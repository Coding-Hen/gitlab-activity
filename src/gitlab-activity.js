...
var GitLabActivity = (function() {
    ...
    var methods = {
        ...
        renderGitLabLink: function(url, title, cssClass) {
            if (!title) {
                title = url;
            }
            if (typeof(cssClass) === 'undefined') cssClass = "";
            return methods.renderLink('https://gitlab.com/' + url, title, cssClass);
        },
        getMessageFor: function(data) {
            methods.getOutputFromRequest("https://gitlab.com/api/v4/projects/" + data.project_id, function(error, output) {
                var renderText = "";
				var d = output;
                var p = data;
				
                data.repoLink = methods.renderGitLabLink(d.path_with_namespace, p.author.name + " / " + d.name);
                data.type = "NoActivity";
				
                // Only show the first 6 characters of the SHA of each commit if given.
                if (p.push_data != null) {
					p.ref_type = p.push_data.ref_type;
                    // Get the branch name if it exists.
                    if (p.push_data.ref != null) {
                        data.branch = p.push_data.ref;
                        data.branchLink = methods.renderGitLabLink(d.path_with_namespace + '/commits/' + p.push_data.ref, p.push_data.ref) + ' at ';
                    }
					if(p.action_name != "deleted"){
                    data.commitsMessage = Mustache.render('<a href="https://gitlab.com/{{repo}}/commit/{{shaDiff}}">{{shortenedRef}}</a> · {{commit_title}}', {
                        repo: d.path_with_namespace,
                        shaDiff: p.push_data.commit_to,
						shortenedRef: p.push_data.commit_to.substr(0,8),
						commit_title: p.push_data.commit_title
                    });
					}
                    if (p.push_data.commit_title != null) {
                        if (p.push_data.commit_title.length > 66) {
                            p.push_data.commit_title = p.push_data.commit_title.substring(0, 66) + '...';
                        }
				...
				if(p.action_name == "opened"){
					data.type = "IssuesEvent";
                    data.issueLink = methods.renderLink(d.web_url + "/issues/" + p.target_iid, "issue #" + p.target_iid);
					
				}
                if (p.action_name == "commented on") {
                    data.type = "IssueCommentEvent";
                    var title = d.path_with_namespace + "#" + p.note.noteable_iid;
                    data.issueLink = methods.renderLink(d.web_url + "/issues/" + p.note.noteable_iid, title);
                    data.issueType = "issue";
                    data.comment = p.note.body;
                }
                ...
                if (p.action_name === 'FollowEvent') data.targetLink = methods.renderGitLabLink(p.target.login);
                ...
                if (p.action_name == 'created' || p.action_name == "pushed new" || p.action_name == "imported") {
					...
					if(p.action_name == "imported"){
                        ...
                        icon = icons[data.type + '_' + p.push_data.ref_type];
                    }
                } else {
                ...
                var timeString = millisecondsToStr(new Date() - new Date(p.created_at));
                var activity = {
                    message: message,
                    icon: icon,
                    timeString: timeString,
                    userLink: methods.renderGitLabLink(p.author_username, p.author.name)
                };

                if (singleLineActivities.indexOf(data.type) > -1) {

                    renderText += Mustache.render(templates.SingleLineActivity, activity);
                } else {
                    renderText += Mustache.render(templates.Activity, activity);
                }
                renderData[p.created_at] = renderText;
                if (lastItem) {
                    methods.renderIfReady(selector, header, renderData);
                }
            });

        },
        getHeaderHTML: function(data) {
            data.userLink = methods.renderLink(data.web_url, data.username);
            data.gravatarLink = methods.renderLink(data.web_url, '<img src="' + data.avatar_url + '">');
        ...
        renderIfReady: function(selector, header, activity) {
            if (header && activity && lastItem) {
                var texta = "";
                Object.keys(activity).sort().reverse().forEach(function(v, i) {
                    texta += activity[v];
                });
                methods.writeOutput(selector, header + texta);
            }
        }
    };
    ...
    obj.feed = function(options) {
        ...
        var userUrl = 'https://gitlab.com/api/v4/users?username=' + options.username,
            eventsUrl = 'https://gitlab.com/api/v4/users/' + options.username + '/events',
            activity;

        if (!!options.repository) {
            eventsUrl = 'https://gitlab.com/api/v4/users/' + options.username + '/' + options.repository + '/events';
        }
